...
def main():
    ...
    log_file_test = open(os.path.join(args.log, 'loss_test_log.txt'), 'w' if args.last_epoch == -1 else 'a')
    ...
    for epoch in range(start, end):
        ...
        log = f'Epoch {epoch}, Test Loss: {mtest_loss:.4f}, ' + \
              f'Test MAPE: {mtest_mape:.4f}, ' + \
              f'Test MSE: {mtest_mse:.4f}'
        print(log)
        
        log_file_test.write(log + '\n')
        log_file_test.flush()

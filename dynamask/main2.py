...
class SimpleSaliencyGenerator(nn.Module):
    def __init__(self, time_steps, num_nodes, channels, hidden_channels = 32):
        super(SimpleSaliencyGenerator, self).__init__()
        
        self.conv1 = nn.Conv2d(channels, hidden_channels, kernel_size=(1, 1))
        self.conv2 = nn.Conv2d(hidden_channels, channels, kernel_size=(1, 1))
        self.gate = nn.Conv2d(channels, 1, kernel_size=(1, 1))
        
        self.filter = nn.Conv2d(channels, 1, kernel_size=(1, 1))
        
    def forward(self, x: Tensor, previous_mask: Tensor):
        # x: [time_steps, num_nodes, channels]
        # previous_mask: [time_steps, num_nodes]
        
        # [batch_size, channels, num_nodes, time_steps]
        x = x.unsqueeze(dim=0).transpose(-1, -3)
        previous_mask = previous_mask.unsqueeze(dim=-1).unsqueeze(dim=0).transpose(-1, -3)
        
        g = torch.relu(self.conv1(x))
        g = torch.relu(self.conv2(g))
        g = torch.tanh(self.gate(g))
        
        f = torch.sigmoid(self.filter(x))
        
        # [batch_size, channels, num_nodes, time_steps]
        mask: Tensor = f * previous_mask + (1 - f) * g
        # [time_steps, num_nodes]
        mask: Tensor = mask.transpose(-1, -3).squeeze(dim=-1).squeeze(dim=0)
...
def main():
    ...
    saliencyGenerator = SimpleSaliencyGenerator(args.forecast_length, args.num_nodes, args.in_dims)
    saliencyGenerator.to(device)
    ...
    for i, sample in enumerate(samples):
        ...
        optimizer = optim.Adam(saliencyGenerator.parameters(), 
